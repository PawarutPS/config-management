...
from dynamodb_config_manager.config import load_env_config, load_table_map
from dynamodb_config_manager.dynamodb_client import DynamoDBConfigClient
from dynamodb_config_manager.models import ConfigManagerError, DeploymentSummary, Environment, Scope
...
def deploy(
    ...
    _run_deploy(
        ...
        delete_removed_tables=delete_removed_tables,
        confirm_delete_tables=confirm_delete_tables,
        repo_root=repo_root,
    )
...
def _run_deploy(
    ...
    backup_s3_prefix: str,
    delete_removed_tables: bool,
    confirm_delete_tables: bool,
    repo_root: Path,
) -> None:
    logging.basicConfig(
        level=logging.INFO,
    ...
    try:
        ...
        resolved_table_map = Path(table_map) if table_map is not None else None
        loaded_env_config = load_env_config(env=None, config_path=None)
        loaded_table_map = load_table_map(resolved_table_map)
...
def print_summary(summary: DeploymentSummary) -> None:
    ...
    tables_deleted = sum(1 for result in summary.results if result.table_deleted)
